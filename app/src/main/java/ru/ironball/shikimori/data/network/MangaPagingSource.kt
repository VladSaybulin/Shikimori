...
class MangaPagingSource @AssistedInject constructor(
    ...
    @AssistedFactory
    interface Factory {
        fun create(@Assisted filters: Map<String, String>): MangaPagingSource
    }
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Manga> {
        try {
            ...
            if (response.isSuccessful) {
                val mangas = response.body()!!.map { it.toManga() }
                val nextPageNumber = if (mangas.isEmpty()) null else pageNumber + 1
